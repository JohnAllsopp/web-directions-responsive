var geoip = {
	...
	getCountryCode: function(){
		...
		if (!geoip.countryHasPricing(cc)){
			return "DF" //country that doesn't have its own currency, return default
		}
		else {
			return cc
		}
	...
	getDiscountCode: function(){
		return document.querySelector("#discountCode").value.toLowerCase();
	},
	...
	getIndividualPrice: function(){
		...
		if (geoip.hasValidDiscountCode(countryCode)) {
			...
			if (geoip.getDiscountCode() == "techtalks") {
				//special case for techtalks
				discount = "TechTalks"
			}
			else {
				discount = "Discounted"
	...
	hasValidDiscountCode: function(countryCode) {
		...
		//var discountCode = document.querySelector("#discountCode").value.toLowerCase();
		var discountCode = geoip.getDiscountCode();
	...
	getCountryDiscountCodes: function(countryCode){
		...
		//add them in lowercase, input is lowered!
		
		switch (countryCode) {
			
			case "PH":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera", "pwdo", "devcon", "techtalks", "manilajs"];
			case "MY":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera"];
			case "ID":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera"];
			case "SG":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera", "smd", "sgphp", "sgruby", "singjs"];
			case "HK":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera"];
			case "JP":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera", "wdscode13vip"];
			case "KR":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera"];
			case "TW":
				return ["kendoui", "jscamp",  "mozilla", "html5 rocks", "opera"];
